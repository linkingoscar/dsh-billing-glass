...
//       * 计价政策（峰/谷/平）与更新时间
//   - 轮询 /api/billing-glass/state：余额 60s（服务端缓存），会话切换立即刷新。
	...
	factory: (require) => {
		...
		function BillingGlassCard(props) {
			...
			const onPointerMove = (event) => {
				...
				const nextBottom = clamp(d.baseY - dy, 8, Math.max(8, vh - cardSize.h - 8));
			...
			const modelBadge = activeModel !== null
				? (/pro/i.test(activeModel) ? "Pro" : /flash/i.test(activeModel) ? "Flash" : null)
				: null;
			...
			const cardPos = {
				...
				bottom: clamp(pos.bottom, 8, Math.max(8, vh - cardSize.h - 8))
			};
			const card = {
				...glass,
				position: "absolute",
			...
			const providerRow = (p) => jsxs("div", {
				...
				children: [
					jsxs("span", {
						style: { display: "flex", alignItems: "center", gap: 5, minWidth: 0, color: "var(--dsw-alias-label-secondary)" },
						children: [
							jsx("span", { children: p.displayName }),
