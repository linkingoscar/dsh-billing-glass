...
__export("readMessageCost", readMessageCost);
		...
		__define("prefs", (__exports, __export) => {
 ...
 * @returns {() => void}
 */
function subscribePrefs(notify) {
	listeners.add(notify);
	return () => { listeners.delete(notify); };
}

__export("loadPrefs", loadPrefs);
__export("setPref", setPref);
__export("subscribePrefs", subscribePrefs);
		});
...
const { loadPrefs, subscribePrefs } = __modules["prefs"];
...
__export("MessageCostChip", MessageCostChip);
		...
		__define("settings-section", (__exports, __export) => {
// 设置卡片（Harness settings.section 槽位，v0.1.0-rc.7+ 可用；更早宿主上
// slots.inject 静默等待声明，不会报错——优雅降级为无此设置页）。
const { useState, useEffect, jsx, jsxs } = __modules["runtime"];
const { loadPrefs, setPref, subscribePrefs } = __modules["prefs"];
const { POS_KEY } = __modules["constants"];
const rowStyle = {
...
function BillingGlassSettingsSection() {
	const [prefs, setPrefs] = useState(loadPrefs);
	useEffect(() => subscribePrefs(setPrefs), []);
	/**
	 * @param {"capsule"|"costChip"} key
...
__export("BillingGlassSettingsSection", BillingGlassSettingsSection);
		});
...
const { BillingGlassSettingsSection } = __modules["settings-section"];
const inject = ["slots"];
