 ...
 * @property {{source:string,mode:string,cny:{input:number,cacheRead:number,output:number},usd:{input:number,cacheRead:number,output:number}}|null} pricingSnapshot
 */
...
function normalizeEntry(raw) {
  ...
  const bucket = entry.bucketCostNative;
  const bucketCostNative = bucket !== null && typeof bucket === "object"
    && finite(bucket.input) && finite(bucket.cacheRead) && finite(bucket.output)
    ? { input: bucket.input, cacheRead: bucket.cacheRead, output: bucket.output }
    : null;
  const costNative = finite(entry.costNative) ? /** @type {number} */ (entry.costNative) : legacyCostNative;
  const tokenTotal = (finite(entry.inputTokens) ? entry.inputTokens : 0)
    + (finite(entry.cacheReadTokens) ? entry.cacheReadTokens : 0)
    + (finite(entry.outputTokens) ? entry.outputTokens : 0);
  const legacyBucket = tokenTotal > 0
    ? {
        input: costNative * (finite(entry.inputTokens) ? entry.inputTokens : 0) / tokenTotal,
        cacheRead: costNative * (finite(entry.cacheReadTokens) ? entry.cacheReadTokens : 0) / tokenTotal,
        output: costNative * (finite(entry.outputTokens) ? entry.outputTokens : 0) / tokenTotal
      }
    : { input: 0, cacheRead: 0, output: costNative };
  const snap = entry.pricingSnapshot;
  const pricingSnapshot = snap !== null && typeof snap === "object"
    && typeof snap.source === "string" && snap.cny !== null && typeof snap.cny === "object"
    && snap.usd !== null && typeof snap.usd === "object"
    ? snap
    : null;
  return {
    ...
    bucketCostNative: bucketCostNative ?? legacyBucket,
    pricingSnapshot
  };
}
...
export function createLedger(ctx, options = {}) {
    ...
    try {
      ...
      if (parsed !== null && typeof parsed === "object" && Array.isArray(parsed.records)) {
        for (const entry of parsed.records) {
          ...
          if (normalized !== null) loadRecord(normalized);
        }
  ...
  /** 回放专用：已有账本记录即保持其历史价格快照，不用当前目录覆盖。
   * @param {LedgerEntry} entry
   */
  function recordIfAbsent(entry) {
    if (disposed) return false;
    const normalized = normalizeEntry(entry);
    if (normalized === null) return false;
    const key = `${normalized.sessionId}\u0000${normalized.messageId}`;
  ...
  function querySession(sessionId) {
    const found = [];
    for (const key of sessionKeys.get(sessionId) ?? []) {
      const entry = records.get(key);
  ...
  function summary(now = new Date(), timezone = "local") {
    ...
    const cacheKey = `${timezone}|${keys.day}|${recordsRevision}`;
  ...
  return { record, recordIfAbsent, querySession, queryMessage, summary, health: healthSnapshot, flushSync: writeNow, dispose };
}
