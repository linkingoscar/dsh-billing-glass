...
export function createLedger(ctx, options = {}) {
  ...
  const health = { invalidLines: 0, recoveredTail: 0, degraded: false };
  let timer = null;
  let dirty = false;
  let lineCount = 0;
  let disposed = false;
  ...
  function loadJsonl() {
    ...
    // 尾部半条记录：截到最后一个完整换行，启动后由 compact 重写修复。
    let content = raw;
    if (!content.endsWith("\n")) {
      const lastNewline = content.lastIndexOf("\n");
      health.recoveredTail = 1;
    ...
    for (const line of content.split("\n")) {
      ...
      let entry;
      try {
        entry = normalizeEntry(JSON.parse(line));
      } catch {
        entry = null;
      }
      if (entry === null) {
  ...
  const jsonlFound = loadJsonl();
  if (!jsonlFound) {
    loadLegacy();
    if (dirty) scheduleWrite();
  }
  // 启动时发现损坏/尾部半条记录：立即用有效记录重写文件修复。
  if (health.degraded) compact();
