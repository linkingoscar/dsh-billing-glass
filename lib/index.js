...
/** 空的会话费用记录（扁平合计 + 分桶明细，金额分原生币种与 USD）。 */
 ...
 * 模型无价格时返回 `{ priced: false, unpricedReason: "pricing_unknown", ... }`：
 * 计费系统 fail closed——绝不静默按 0 元计费。
 */
function priceEventInto(record, event, provider) {
  ...
  const nativeCurrency = sample.nativeCurrency ?? provider.currency ?? "USD";
  const unitNative = nativeUnitOf(unit, nativeCurrency);
  record.calls += 1;
  record.costNative += sample.costNative;
  record.nativeCurrency ??= nativeCurrency;
  ...
  record.buckets.input.costNative += (sample.inputTokens * unitNative.input) / 1e6;
  record.buckets.cacheRead.tokens += sample.cacheReadTokens;
  record.buckets.cacheRead.costNative += (sample.cacheReadTokens * unitNative.cacheRead) / 1e6;
  record.buckets.output.tokens += sample.outputTokens;
  record.buckets.output.costNative += (sample.outputTokens * unitNative.output) / 1e6;
  return { ...sample, model, priced: true };
}
...
function apply(ctx) {
    ...
    try {
      ...
      for (const line of raw.content.split("\n")) {
        ...
        try {
          ...
          recordSample(sessionId, event.data?.message?.id, provider, sample, event.time ?? Date.now());
        } catch { /* 单条坏消息不阻断整段回放 */ }
      }
  ...
  ctx.on("session/event", (session, event) => {
    try {
      ...
      // 实时兜底也沿用最近一次 request/header：source 缺 provider/baseURL 时仍能归属。
      const header = entry.lastHeader;
      const provider = matchProvider(
        source?.provider ?? header?.provider,
        extractBaseUrl(source) ?? extractBaseUrl(header)
      );
      ...
      recordSample(session.id, messageId, provider, sample, event.time ?? Date.now());
      entry.lastProvider = provider.id;
    } catch (error) {
  ...
  async function todayOf(provider, balanceData) {
    if (balanceData === null || typeof provider.todayConsumed !== "function") return null;
    const now = Date.now();
    const hit = todayCache.get(provider.id);
    if (hit !== void 0 && now - hit.at < TODAY_TTL_MS) return hit.value;
    const inflight = todayInflight.get(provider.id);
    if (inflight !== void 0) return inflight;
    const promise = (async () => {
      try {
        const result = await provider.todayConsumed(ctx, {}, balanceData);
    ...
    todayInflight.set(provider.id, promise);
    try {
      const value = await promise;
      todayCache.set(provider.id, { at: Date.now(), value });
      return value;
    } finally {
      if (todayInflight.get(provider.id) === promise) todayInflight.delete(provider.id);
    }
  }
      ...
      handler: async (req, res) => {
        try {
          ...
          const providerRows = await Promise.all(PROVIDERS.map(async (provider) => {
            ...
            const today = await todayOf(provider, balanceEntry.data);
