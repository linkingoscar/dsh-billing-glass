...
/** 把计价样本（含"未计价"样本）写入账本，幂等由 ledger 保证。 */
function recordSample(sessionId, messageId, provider, sample, time) {
  if (ledger === null || sample === null || typeof messageId !== "string" || messageId === "") return;
    ...
    () => ctx.webServer.register({
      ...
      handler: async (req, res) => {
        try {
          ...
          const forceBalance = url.searchParams.get("force") === "1";
          const forceProviderId = url.searchParams.get("providerId") ?? null;
          ...
          const providerRows = await Promise.all(PROVIDERS.map(async (provider) => {
            const balanceEntry = await balanceOf(
              provider,
              forceBalance && (forceProviderId === null || forceProviderId === provider.id)
            );
