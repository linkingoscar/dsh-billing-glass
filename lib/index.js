...
import { PI_AI_CATALOG_META } from "./providers/openai-compat.js";
...
function apply(ctx) {
  ...
  /** @template T @param {Map<string, T>} map @param {string} key @param {T} value */
  function setSessionCache(map, key, value) {
    map.delete(key);
    map.set(key, value);
  ...
  /** @param {any} entry 已归一化的持久化账本记录。 */
  function ledgerSample(entry) {
    return {
      priced: entry.priced !== false,
      unpricedReason: entry.unpricedReason ?? undefined,
      model: entry.model,
      ...
      bucketCostNative: entry.bucketCostNative,
      pricingSnapshot: entry.pricingSnapshot ?? undefined
    };
  }
   ...
   * @param {number} time
   */
  function recordSample(sessionId, messageId, provider, sample, time, preserveExisting = false) {
    ...
    const entry = {
      ...
      bucketCostNative: sample.bucketCostNative ?? { input: 0, cacheRead: 0, output: sample.costNative },
      pricingSnapshot: sample.pricingSnapshot ?? null
    };
    if (preserveExisting) ledger.recordIfAbsent(entry);
    else ledger.record(entry);
  }
    ...
    try {
      ...
      for (const line of raw.content.split("\n")) {
        ...
        try {
          ...
          const frozen = typeof messageId === "string" ? ledger.queryMessage(sessionId, messageId) : null;
          const sample = frozen === null
            ? priceEventInto(record, event, context.provider, context)
            : ledgerSample(frozen);
          if (sample !== null) upsertMessageSample(record, messageId, sample);
          recordSample(sessionId, messageId, context.provider, sample, event.time ?? Date.now(), true);
        } catch { /* 单条坏消息不阻断整段回放 */ }
      }
      const result = { revision, at: Date.now(), byProvider };
      setSessionCache(logCostCache, sessionId, result);
      return result;
    } catch (error) {
      ...
      if (event?.type === "request/header") {
        ...
        if (header !== void 0 && header !== null) {
          ...
          if (entry === void 0) {
            ...
            setSessionCache(liveBySession, sessionRecord.id, entry);
          }
      ...
      handler: async (/** @type {{url?: string, method?: string}} */ req, /** @type {any} */ res) => {
        ...
        try {
          ...
          sendJson(res, 200, {
            ...
            pricingCatalog: { source: "pi-ai", version: PI_AI_CATALOG_META.sourceVersion },
