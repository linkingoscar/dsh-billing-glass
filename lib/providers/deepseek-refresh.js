...
function builtinTables() {
  // 现行价 = 2026-05-22 政策（flat）；峰谷 = 2026-08-17 政策（peak/offPeak）。
  const flatPolicy = OFFICIAL_PRICING_POLICIES[1];
  const peakPolicy = OFFICIAL_PRICING_POLICIES[2];
  const models = ["deepseek-v4-flash", "deepseek-v4-pro"];
  const current = {};
  const peak = {};
  for (const model of models) {
    const flat = flatPolicy.prices[model] ?? flatPolicy.prices["*"];
    current[model] = { cacheRead: flat.cny.cacheRead, input: flat.cny.input, output: flat.cny.output };
    const on = peakPolicy.peak[model] ?? peakPolicy.peak["*"];
    const off = peakPolicy.offPeak[model] ?? peakPolicy.offPeak["*"];
    peak[model] = {
