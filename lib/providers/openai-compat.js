...
export function defineOpenAiCompatProvider(cfg) {
  ...
  return {
    ...
    priceAt(model, _timeMs) {
      // fail closed：目录里没有这个模型（也没有 `*` 兜底价）时返回 null，
      // 调用方标记"暂无价格"，绝不按 0 元静默计费。
      if (typeof model !== "string" || model === "") return null;
      let entry = prices[model];
      if (entry === void 0) {
        // 网关常见别名：openai/gpt-5.6-luna → gpt-5.6-luna；再尝试大小写不敏感匹配。
        const bare = model.split("/").at(-1) ?? model;
        entry = prices[bare];
      }
      if (entry === void 0) {
        const lower = model.toLowerCase();
        const key = Object.keys(prices).find((candidate) => candidate.toLowerCase() === lower);
        if (key !== void 0) entry = prices[key];
      }
