...
function loadDayState(path) {
  try {
    ...
    if (
      ...
      // consumed 是新增字段；旧版日状态没有该字段时由 estimateTodayConsumed 迁移。
      return parsed;
    }
  } catch {}
  return null;
 ...
 * 余额差估算今日消费（元，四舍五入到分）。
 *
 * 关键：按“每次余额下降的累计值”计算，而不是 `opening − current`。
 * 这样当天用完再充值不会把已发生的消费抹掉：
 *   last=100 → 用光 last=0（累计 +100）→ 充值 balance=100（last 更新为 100）
 *   → 又用掉 20（累计 +20）→ 今日消费 = 120。
 */
function estimateTodayConsumed(ctx, balance) {
  ...
  if (stored !== null && stored.date === today) {
    // 旧状态迁移：没有累计字段时，用 opening 与上次 last 之间的差额作为已发生消费。
    const carried = Number.isFinite(stored.consumed)
      ? stored.consumed
      : Math.max(0, stored.opening - stored.last);
    const delta = Math.max(0, stored.last - balance);
    state = {
      date: today,
      opening: stored.opening,
      last: balance,
      consumed: Math.round((carried + delta) * 100) / 100
    };
  } else {
    // 新的一天：期初余额取昨天最后一次余额（没有历史就用当前余额，今日消费从 0 开始）。
    const opening = stored !== null ? stored.last : balance;
    state = {
      date: today,
      opening,
      last: balance,
      consumed: Math.round(Math.max(0, opening - balance) * 100) / 100
    };
  }
  saveDayState(path, state);
  return Math.round(state.consumed * 100) / 100;
}
