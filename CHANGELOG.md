...
### Added
- 每条账本记录持久化首次计价的目录来源、单价与 token 桶小计；历史回放复用
  该快照，目录升级不再重算并改写旧消费。
- state 响应与卡片展示 pi-ai 价格目录版本，明确费用为本地估算。

### Changed
- 上游目录中三类 token 单价全为 0 的模型改判为订阅/额度套餐并标记未计价，
  不再显示成「免费按量」。
- 会话回放缓存和浏览器消息缓存增加容量上限；账本按 session 建索引并缓存
  日/月/总计聚合，避免轮询时重复全表扫描。
